...
- 从模板菜单中获取
  ...
  若你已经拥有模板，可以直接在 Word 的菜单 “毕业论文” 页面，找到 “获取更新” 下拉菜单，从中选择一个版本库连接进行下载。此外，可以通过 “检查更新” 按钮，从 Gitee 上检查是否有新的发布版本。

  ![Word菜单截图](Documents/Images/MenuFull.png)

- 更新模板

  当你已经下载了更新的论文模板，可通过 “毕业论文” 页面下的 “替换论文模板” 按钮，完成对模板文件的替换。单击按钮后，在弹出的文件选择窗口中，定位到更新的论文模板文件，选定后完成替换。

  **注意：** 直接下载的论文模板文件，通常包含有版本信息，例如 “成都信息工程大学学士学位论文模板 v1.2.0.dotm”。此时会检查选中的模板文件的版本，如无法从模板文件中发现任何版本信息，或者版本较低，则将弹出警告信息。

### 论文模板样式说明

模版根据《成都信息工程大学学士学位论文规范》文件，定义了如下样式。

![模板所有自定义样式](Documents/Images/StyleList.png)

其中，“论文”开始的样式为自定义样式，用户可在撰写论文时直接选用。此外，图中“AMDisplayEquation”和“AMEquationSection”两个样式，是安装并使用了 AxMath（国产公式编辑器，**价格上有优势**，可替代 MathType）之后添加的样式。
...
2.  更新正文页数
    ...
    在论文中，正文页数定义为从 “第一章 绪论” 开始到 “致谢” 结束的页数。论文中有两个地方涉及到正文页数。一个是目录页上的 “论文总页数” 标记。
...
3.  删除多余空格
    ...
    当从网页或其他程序中拷贝内容到论文后，有时会在中英文之间、中文标点符号前后、段落开始和结束前后插入额外的空格。此时，可选中需要清楚空格的段落文字，单击该按钮，可清楚多余空格。

    **注意：** 本功能不会删除英文字符间的空格，因此不用担心将英文内容的样式弄乱。**若不选择任何内容，将对整个论文进行替换，请谨慎使用。**

4.  替换英文标点

    当需要替换中文字符间错误键入的英文标点时，可以直接选中可能出现的区域，单击该按钮实现。

    **注意：** 本功能不会替换英文字符间出现的英文标点，因此不用担心将英文内容的样式弄乱。**若不选择任何内容，将对整个论文进行替换，请谨慎使用。**
...
7.  插入算法和算法编号
    ...
    模版提供的算法包括算法名称、输入、输出、伪代码四个部分。若需要在论文中插入整个算法，可单击 “插入算法模板” 按钮。此时，模版将自动插入一个带编号的算法模板，如下图所示。

    ![插入算法](Documents/Images/AlgorithmExample.png#pic_center)

    之后，可在算法编号后面键入标题，并在表格中键入相应内容。作者也可以根据需要，自行对算法表格进行调整。

    如希望像图和表一样，仅插入算法编号，可单击 “插入算法编号” 按钮。
...
如果重新搭建一个全新的开发环境，可能会遇到一些问题。

1. Pycharm 中无法运行 makeVBAProjectFile.py

    这个通常是因为本地安装的 Python 版本和代码库中虚拟环境（.venv）中的版本不一致导致的。可以删除工程中的 .venv 目录，根据本地 Python 重新生成一个新的虚拟环境即可解决问题。

2. 运行 makeVBAProjectFile.py 时报错 “'Microsoft Word', '到 Visual Basic Project 的程序访问不被信任。”

    这个通常是因为 Word 的安全设置阻止了对VBA项目的访问。可以通过如下步骤解决：
        - 打开Word应用程序
    ...
    如 ![下图](Documents/Images/FixProbVBNotTrusted.png) 所示。
